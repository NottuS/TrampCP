...
<c:url var='link_pesquisar' value='/tipoDocumentacao.do?action=pesquisarTipoDocumentacao' />
...
<script language="javascript">
	...
	function voltar(){	
		document.tipoDocumentacaoForm.descricao.value = "";
		document.tipoDocumentacaoForm.action="${link_pesquisar}";
		document.tipoDocumentacaoForm.submit();
	}
	
	function validarCampos(){
	
		var form = document.tipoDocumentacaoForm;
		Trim(form.descricao);
	...
	<c:choose> 
		<c:when test='${tipoDocumentacaoForm.actionType == "incluir"}'>
			<c:set var="acao" value="tipoDocumentacao.do?action=salvarTipoDocumentacao"></c:set>
			<c:set var="titulo" value="Incluir"></c:set>
		</c:when>
		<c:otherwise>
			<c:set var="acao" value="tipoDocumentacao.do?action=alterarTipoDocumentacao"></c:set>
...
<div id="conteudo">
	<div align="center">
	<h1>${titulo} Tipo de Documentação</h1>

	<html:form action="${acao}" onsubmit="return validarCampos()">
	<html:hidden property="codTipoDocumentacao" value="${tipoDocumentacaoForm.codTipoDocumentacao}"/>
	<html:hidden property="actionType" value="${tipoDocumentacaoForm.actionType}" />
