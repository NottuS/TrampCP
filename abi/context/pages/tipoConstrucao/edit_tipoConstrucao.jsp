...
<c:url var='link_pesquisar' value='/tipoConstrucao.do?action=pesquisarTipoConstrucao' />
...
<script language="javascript">
	...
	function voltar(){	
		document.tipoConstrucaoForm.descricao.value="";
		document.tipoConstrucaoForm.action="${link_pesquisar}";
		document.tipoConstrucaoForm.submit();
	}
	
	function validarCampos(){
	
		var form = document.tipoConstrucaoForm;
		Trim(form.descricao);
	...
	<c:choose> 
		<c:when test='${tipoConstrucaoForm.actionType == "incluir"}'>
			<c:set var="acao" value="tipoConstrucao.do?action=salvarTipoConstrucao"></c:set>
			<c:set var="titulo" value="Incluir"></c:set>
		</c:when>
		<c:otherwise>
			<c:set var="acao" value="tipoConstrucao.do?action=alterarTipoConstrucao"></c:set>
...
<div id="conteudo">
	<div align="center">
	<h1>${titulo} Tipo de Construção</h1>

	<html:form action="${acao}" onsubmit="return validarCampos()">
	<html:hidden property="codTipoConstrucao" value="${tipoConstrucaoForm.codTipoConstrucao}"/>
	<html:hidden property="actionType" value="${tipoConstrucaoForm.actionType}" />
